...
export class AccountManager {
  ...
  _loadAccounts() {
    ...
    if (fileAccounts.length > 0) {
      this.logger.info(`Loaded ${fileAccounts.length} accounts from ${this.storageFile}`);
    }
  }

  _loadAccountsFromFile() {
    try {
      ...
      if (file) {
        ...
        if (content) {
          ...
          return Array.isArray(data) ? data.map(user => User.fromJSON(user)) : [];
        }
      }
    } catch (error) {
    }
